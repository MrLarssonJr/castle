use crate::model::{BasicCredential, SessionToken};
...
impl Authentication {
	async fn from_basic(db: &PgPool, value: &str) -> Result<Authentication, ApiError> {
		...
		let Some(user_id) = store::users::authenticate(db, &creds)
			.await
	...
	async fn from_session(
		...
		let Some(user_id) = store::sessions::authenticate(db, session_token)
			.await
...
impl FromRequestParts<AppState> for Authentication {
	...
	async fn from_request_parts(
		...
		let auth_header = auth_header.to_str().map_err(|err| {
			tracing::warn!("bad authorization header: {}", err.to_pretty_string());
			ApiError::BadRequest
		})?;
