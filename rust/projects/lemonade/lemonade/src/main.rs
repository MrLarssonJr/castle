...
use lemonade_db::nordigen_tokens::TokenUpdater;
...
async fn main() {
	...
	tokio::spawn({
		...
		async move {
			let Err(err): Result<_, Whatever> =
				lemonade_db::nordigen_tokens::keep_access_token_update(
					&pool,
					secret_id,
					access_token,
				)
	...
	tokio::spawn({
		...
		async move {
			let updater = updater;
			let Err(err): Result<_, Whatever> =
				lemonade_db::nordigen_tokens::keep_token_updated(&pool, secret, &updater)
					.await
